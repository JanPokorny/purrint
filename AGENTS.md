...
- **Entry point (`src/main.tsx`)** mounts `PurrintApp`, applies the global styles in `src/index.css` (including a noisy texture background), and loads the IBM VGA bitmap font (for preview/text) and Space Grotesk/Patrick Hand (for UI).
...
- **Assets** include the embedded WOFF of the IBM VGA font (`src/assets/WebPlus_IBM_VGA_9x16.woff`) referenced both by CSS and UI elements.

## UI & Interaction Flow
1. Initial render shows the clean, modern UI with a noisy texture background, compatibility notice (only when `navigator.bluetooth` is missing), the Image/Text toggle, and a receipt preview area. The primary CTA is disabled until Web Bluetooth is supported.
2. **Image mode**  
   ...
   - Clicking `purrint!` (the rotated button) sends the stored bitmap to `printImage`.
3. **Text mode**  
...
- Global styling resides in `src/index.css`, providing the clean, playful aesthetic with drop shadows, noisy texture, and "Patrick Hand" typography.
- `vite.config.ts` wires `@vitejs/plugin-react` and `vite-plugin-pwa` to provide offline capability and a basic manifest.
